  ...
  <div class="contentBox">
    ...
    <div class="contentBoxCont">
      <div class="mgb20">
        ...
        <el-button style="float: right" type="primary" @click="go(['guideEdit','create'])" icon="plus">发布办事指南</el-button>
      </div>
      <div class="tableList mgb20">
        <MyTable :data="list">
          ...
          <MyColumn prop="files" label="文件类型" :formatter="({subType})=>typeFilter(articleType.guide,subType)" min-width="110"/>
          ...
          <MyColumn label="操作" width="120px">
            <template scope="scope">
              <el-button type="text" size="small" @click="go(['guideEdit',scope.row.id])"> 编辑</el-button>
...
<script type="es6">
  ...
  export default {
    ...
    methods: {
      ...
      ...filter,
      del(idx, {id, name}) {
        confirm(`确定删除办事指南[${name}]吗?`, 'warning').then(() => this.delArticle([id, idx]))
      },
      change(key, value){
        this.changeSelect({key, value});
        this.findArticle([8,0])
      },
      search(){
