...
<template>
  <div class="contentBox">
    <div class="contentBoxtitle"><span>活动展示列表</span></div>
    <div class="contentBoxCont">
      <div class="mgb20">
        <MySelect title="活动类型" field="subType" :options="articleType.activity || {}" :change="change"/>
        ...
        <el-button style="float: right" type="primary" @click="go(['actEdit','create'])" icon="plus">发布活动展示</el-button>
      </div>
      <div class="tableList mgb20">
        <MyTable :data="list">
          ...
          <MyColumn prop="files" label="活动类型" :formatter="({subType})=>typeFilter(articleType.activity,subType)" min-width="110"/>
          ...
          <MyColumn label="操作" width="120px">
            <template scope="scope">
              <el-button type="text" size="small" @click="go(['actEdit',scope.row.id])"> 编辑</el-button>
      ...
      <div class="pageSlide">
        <MyPagination :method="()=>findArticle([7, 0])"/>
      </div>
    </div>
  </div>
</template>
<script type="es6">
  ...
  export default {
    ...
    methods: {
      ...
      ...filter,
      del(idx, {id, name}) {
        confirm(`确定删除活动展示[${name}]吗?`, 'warning').then(() => this.delArticle([id, idx]))
      },
      change(key, value){
        this.changeSelect({key, value});
        this.findArticle([7, 0])
      },
      search(){
