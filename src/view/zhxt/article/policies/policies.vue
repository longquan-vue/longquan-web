...
<template>
  <div class="contentBox">
    <div class="contentBoxtitle"><span>政策法规列表</span></div>
    <div class="contentBoxCont">
      <div class="mgb20">
        ...
        <el-button style="float: right" type="primary" @click="go(['policiesEdit','create'])" icon="plus">发布政策法规</el-button>
      </div>
        ...
        <MyTable :data="list">
          ...
          <MyColumn label="操作" width="120px">
            <template scope="scope">
              <el-button type="text" size="small" @click="go(['policiesEdit',scope.row.id])"> 编辑</el-button>
      ...
      <div class="pageSlide">
        <MyPagination :method="()=>findArticle([3, 0])"/>
      </div>
    </div>
  </div>
</template>
<script type="es6">
  ...
  export default {
    ...
    methods: {
      ...
      ...filter,
      del(idx, {id, name}) {
        confirm(`确定删除政策法规[${name}]吗?`, 'warning').then(() => this.delArticle([id, idx]))
      },
      change(key, value){
        this.changeSelect({key, value});
        this.findArticle([3, 0])
      },
      search(){
