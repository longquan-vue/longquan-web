...
<template>
  <div class="contentBox">
    <div class="contentBoxtitle"><span>工会服务列表</span></div>
    <div class="contentBoxCont">
      <div class="mgb20">
        <MySelect title="服务类型" field="subType" label="name" :options="articleType.service || {}" :change="change"/>
        ...
        <el-button style="float: right" type="primary" @click="go(['serviceEdit','create'])" icon="plus">发布工会服务</el-button>
      </div>
      <div class="tableList mgb20">
        <MyTable :data="list">
          ...
          <MyColumn prop="files" label="服务类型" :formatter="({subType})=>type2Filter(articleType.service,subType)" min-width="110"/>
          ...
          <MyColumn prop="adminName" label="发布者" min-width="140"/>
          <MyColumn prop="created" label="发布时间" :formatter="({created})=>date3Filter(created)" min-width="170"/>
          <MyColumn label="操作" width="120px">
            <template scope="scope">
              <el-button type="text" size="small" @click="go(['serviceEdit',scope.row.id])"> 编辑</el-button>
      ...
      <div class="pageSlide">
        <MyPagination :method="()=>findArticle([2, 0])"/>
      </div>
    </div>
  </div>
</template>
<script type="es6">
  ...
  export default {
    ...
    methods: {
      ...
      ...filter,
      del(idx, {id, name}) {
        confirm(`确定删除工会服务[${name}]吗?`, 'warning').then(() => this.delArticle([id, idx]))
      },
      change(key, value){
