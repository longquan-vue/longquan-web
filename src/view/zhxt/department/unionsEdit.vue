      ...
      <el-form :model="data" :rules="rules" ref="ruleForm" label-width="130px">
        <div class="dep_info">
          ...
          <el-form-item label="公会名称：" prop="name">
            <el-input :value="data.name" @input="(v)=>setData({name:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          ...
          <el-form-item label="单位类型：" prop="type">
            <el-input :value="data.type" @input="(v)=>setData({type:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="所属行业：" prop="industry">
            <el-input :value="data.industry" @input="(v)=>setData({industry:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          ...
          <el-form-item label="单位地址：" prop="address">
            <el-input :value="data.address" @input="(v)=>setData({address:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="法人登记号：" prop="corporation">
            <el-input :value="data.corporation" @input="(v)=>setData({corporation:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="法人电话：" prop="corPhone">
            <el-input :value="data.corPhone" @input="(v)=>setData({corPhone:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="邮编：" prop="code">
            <el-input :value="data.code" @input="(v)=>setData({code:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item/>
          <el-form-item label="职工总数：" prop="workers">
            <el-input :value="data.workers" @input="(v)=>setData({workers:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="女职工总数：" prop="feWorkers">
            <el-input :value="data.feWorkers" @input="(v)=>setData({feWorkers:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="会员总数：" prop="users">
            <el-input :value="data.users" @input="(v)=>setData({users:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="女会员总数：" prop="feUsers">
            <el-input :value="data.feUsers" @input="(v)=>setData({feUsers:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="分工会数：" prop="childs">
            <el-input :value="data.childs" @input="(v)=>setData({childs:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="分工会小组数：" prop="groups">
            <el-input :value="data.groups" @input="(v)=>setData({groups:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="届数：" prop="terms">
            <el-input :value="data.terms" @input="(v)=>setData({terms:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          ...
          <el-form-item label="主席姓名：" prop="chaiName">
            <el-input :value="data.chaiName" @input="(v)=>setData({chaiName:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="本届任期：" prop="term">
            <el-input :value="data.term" @input="(v)=>setData({term:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="行政职务：" prop="duties">
            <el-input :value="data.duties" @input="(v)=>setData({duties:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="主席电话：" prop="chaiPhone">
            <el-input :value="data.chaiPhone" @input="(v)=>setData({chaiPhone:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="主席手机号：" prop="chaiTel">
            <el-input :value="data.chaiTel" @input="(v)=>setData({chaiTel:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="专（兼）职：" prop="job">
            <el-input :value="data.job" @input="(v)=>setData({job:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          ...
          <el-form-item label="职工之家等级：" prop="leve">
            <el-input :value="data.leve" @input="(v)=>setData({leve:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
        </div>
        <div class="dep_info">
          <div class="info"><span>申请人信息</span></div>
          <el-form-item label="申请人姓名：" prop="applyName">
            <el-input :value="data.applyName" @input="(v)=>setData({applyName:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="电话：" prop="applyPhone">
            <el-input :value="data.applyPhone" @input="(v)=>setData({applyPhone:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item label="邮箱地址：" prop="applyEmail">
            <el-input :value="data.applyEmail" @input="(v)=>setData({applyEmail:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
          <el-form-item/>
          <el-form-item label="申请备注：" prop="applyMark" style="width: 99.5%">
            <el-input :value="data.applyMark" type="textarea" :rows="5" @input="(v)=>setData({applyMark:v})" :disabled="!data.mine || data.audit == 2"/>
          </el-form-item>
        </div>
        <div class="dep_info">
          <div class="info"><span>审核信息</span></div>
          <el-form-item label="审核状态：" v-if="data.mine" prop="audit">
            <el-input :value="audit2Filter(data.audit)" disabled readonly/>
          </el-form-item>
          ...
          <el-form-item :label="data.mine?'审核失败原因：':'不通过原因：'" prop="fail" style="width: 99.5%">
            <el-input :value="data.fail" :placeholder="data.mine?'无':'请输入失败原因...'" @input="(v)=>setData({fail:v})" type="textarea" :rows="5" :disabled="data.mine" :readonly="data.mine"/>
          </el-form-item>
          <el-form-item label="审核者：" v-if="data.mine" style="width: 99.5%">
            <el-input :value="data.admin.name || '无'" disabled readonly/>
          </el-form-item>
          <el-form-item label="审核时间：" v-if="data.mine" prop="audTime" style="width: 99.5%">
            <el-input :value="date3Filter(data.audTime)" disabled readonly/>
          </el-form-item>
        </div>
